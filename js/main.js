$(function () {
  ...
  function attachCommonEventListeners() {
    $("#login").on("click", () =>
      $loginModal.css("display", "flex").fadeIn(300)
    );
    ...
    $("#search").on("blur", () =>
      setTimeout(() => $("#suggestions-container").slideUp(), 150)
    );
  ...
  $.get("json/product.json", function (products) {
    ...
    $("#nav-new").click((e) => {
      e.preventDefault();
      renderProducts(allProducts.filter((p) => p.isNew));
    });
    $("#nav-best").click((e) => {
      e.preventDefault();
      renderProducts(allProducts.filter((p) => p.isBest));
    });
    $("#nav-outer").click((e) => {
      e.preventDefault();
      renderProducts(allProducts.filter((p) => p.category === "아우터"));
    });
    $("#nav-top").click((e) => {
      e.preventDefault();
      renderProducts(allProducts.filter((p) => p.category === "상의"));
    });
    $("#nav-bottom").click((e) => {
      e.preventDefault();
      renderProducts(allProducts.filter((p) => p.category === "하의"));
    });
  ...
  $("#darkmode").click(() => {
    ...
    localStorage.setItem(
      "darkMode",
      $("body").hasClass("dark-mode") ? "enabled" : ""
    );
  });
  $(".nav-toggle").click(() =>
