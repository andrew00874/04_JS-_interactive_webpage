...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- 이벤트 리스너 설정 ---
  inputId.addEventListener("input", () => validate(inputId, idResult, regExp.id, "inputId"));
  inputPw.addEventListener("input", () => validate(inputPw, pwResult, regExp.pw, "inputPw"));
  inputName.addEventListener("input", () => validate(inputName, nameResult, regExp.name, "inputName"));
  ...
  signupForm.addEventListener("reset", () => {
    ...
    document.getElementById("idResult").textContent = "영어, 숫자, -, _ 6~16글자 사이";
    document.getElementById("pwResult").textContent = "영어 대/소문자 + 숫자 + 특수문자 (!@#$%^&*) 포함 8~20글자";
    document.getElementById("pwCheckResult").textContent = "비밀번호를 먼저 입력해주세요.";
    document.getElementById("nameResult").textContent = "한글 2~15자 (단모음, 단자음 제외)";
